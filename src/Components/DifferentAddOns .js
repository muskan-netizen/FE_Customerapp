...
const DifferentAddOns = ({
    ...
    const onIncrementDecrement = (item, index, type) => {
        ...
        if (!!storeLocalQty) {
            ...
            if (selectedDiffAdsOnItem?.variant && selectedDiffAdsOnItem?.variant[0]?.check_if_in_cart_app) {
            ...
            <View pointerEvents={btnLoader ? 'none' : 'auto'} style={{}}>
                <View style={styles.flexView}>
                    <View>
                        ...
                        <Text style={styles.titleText}>{currencies?.primary_currency.symbol}{pvariant?.price}</Text>
                    </View>
                ...
                {addon_set.map((val, i) => {
                    return (
                        <View style={{}} key={String(i)}>
                            ...
                            }}>{val?.addon_set_translation_title}</Text>
                            {val.options.map((values, inx) => {
                                return (
                                    <View style={{ marginVertical: 2 }} key={String(inx)}>
                                        ...
                                        }}>{values?.option_translation_title}</Text>
                                    </View>
                                )
                            })}
                        </View>
            ...
            <View style={styles.mainContainer}>
                ...
                <View style={{ paddingHorizontal: moderateScale(12) }}>
                    <FlatList
                        ...
                        ItemSeparatorComponent={() => <View style={{ ...styles.horizontaLine, marginVertical: moderateScaleVertical(8) }} />}
                        ListHeaderComponent={() => <View style={{ height: moderateScale(8) }} />}
                        ListFooterComponent={() => <View style={{ height: moderateScale(40) }} />}
                    />
                </View>
            </View>
...
export default DifferentAddOns;
