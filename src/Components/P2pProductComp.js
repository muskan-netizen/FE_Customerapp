...
export default function P2pProductComp({
    ...
    const LeftImgRightTxt = ({ image, text, isViewDetails = false, isStartChat = false }) => <View style={{
        ...
        <View style={{ flexDirection: "row", padding: moderateScale(6), flex: 0.9 }}>

            <Image source={image} />
            <Text numberOfLines={1} style={styles.rightTxt}>{text}</Text>
        </View>
    ...
    return (
        <View style={{ ...styles.touchContainer, backgroundColor: colors.whiteSmokeColor, }}>
            ...
            {isMoreDetails && <View>
                <LeftImgRightTxt image={imagePath.icTimeOrders} text={moment(item?.products[0]?.start_date_time).format('MMM DD, YYYY hh:mm:A')
                    + " - " + moment(item?.products[0]?.end_date_time).format("MMM DD, YYYY hh:mm:A")} />
                {!isEmpty(item?.products) && !isEmpty(item?.products[0]?.product) && item?.products[0]?.product?.address && <LeftImgRightTxt image={imagePath.icLocationOrders} text={item?.products[0]?.product?.address} />}
