...
function Cart({ navigation, route }) {
  ...
  const { appData, allAddresss, themeColors, currencies, languages, appStyle } = useSelector((state) => state?.initBoot);
  console.log(userData, 'core++ userData');
  ...
  const placeOrder = () => {
    if (!!userData?.auth_token) {
      ...
      if (selectedPayment.id == 24) { //phone number is required in case of cashfree payment method
      ...
      actions
        .openPaymentWebUrl(
          `/${selectedMethod}?tip=${selectedTipAmount && selectedTipAmount != ''
            ? Number(selectedTipAmount)
            : 0
          }&amount=${Number(cartData?.total_payable_amount) +
          (selectedTipAmount != null && selectedTipAmount != ''
            ? Number(selectedTipAmount)
            : 0)
