...
export default function Products({ route, navigation }) {
  ...
  const addSingleItem = async (item) => {
    ...
    updateState({ selectedItemID: item?.id });
    actions
  ...
  const removeProductFromCart = (itemToUpdate) => {
    ...
    actions
      ...
      .then((res) => {
        ...
        updateState({ productListData: updateArray, updateQtyLoader: false, selectedItemID: -1 });
        // showSuccess(res?.message);
      })
      .catch(errorMethod);
  };
  ...
  const errorMethodSecond = (error, addonSet = []) => {
    ...
    if (error?.message?.alert == 1) {
      updateState({ isLoading: false, isLoadingB: false, isLoadingC: false, selectedItemID: -1 });
