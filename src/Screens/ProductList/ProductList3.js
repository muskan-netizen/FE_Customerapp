...
export default function Products({route, navigation}) {
  ...
  const renderSectionItem = ({item, index, section}) => {
    return (
      <View style={{height: moderateScaleVertical(145)}}>
        <ProductCard3
          data={item}
          index={index}
          onPress={moveToNewScreen(navigationStrings.PRODUCTDETAIL, item)}
  ...
  const onScroll = (props) => {
    ...
    // if (
    //   cloneSectionList &&
    //   cloneSectionList.length &&
    //   cloneSectionList.length < 6
    // ) {
    //   return;
    // }
    ...
    cloneSectionList.map((el, index) => {
      if (offset > Number(num.length) * 167) {
        ...
        t = index;
        // activeIdx = index;
        // console.log('onScroll check index', t);
      }
      if (cloneSectionList.length == index + 1) {
        console.log('onScroll check index', t);
        if (activeIdx !== t) {
          console.log('onScroll check index inside', t);
          activeIdx = t;
        }
      }
    });
