...
export default function AllinonePyments({ navigation, route }) {
    ...
    const { themeToggle, themeColor, appStyle, currencies, languages, appData } = useSelector((state) => state?.initBoot);
    ...
    const onNavigationStateChange = (props) => {
        ...
        let transId = '';
        if (url.includes('payment/checkoutSuccess')) { //in case of paypal
            transId = url.substring(url.lastIndexOf('/') + 1)
        } else {
            transId = queryParams?.transaction_id
        }
        setTimeout(() => {
            if (queryParams?.status == 200 || url.includes('payment/checkoutSuccess')) {
    ...
    const subscriptionApiHit = (id) => {
        actions.purchaseSubscriptionPlan(
    ...
    return (
        ...
        >
            ...
            <View style={{ height: moderateScaleVertical(75), backgroundColor: colors.transparent }} />
        </WrapperContainer>
    );
}
