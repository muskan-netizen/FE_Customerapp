...
export default function ProductDetail({route, navigation}) {
  ...
  const onDateChange = val => {
    {
      console.log(val.getMinutes() - startDateRental.getMinutes(), 'val>>>>');
    }
    isRentalEndDatePicker
      ? updateState({
          endDateRental: val,
          rentalProductDuration:Number(
            (val.getDate() - startDateRental.getDate()) * 24 * 60 +
            val.getHours() * 60 -
            startDateRental.getHours() * 60 +
            (val.getMinutes() - startDateRental.getMinutes())),
        })
      : updateState({
          startDateRental: val,
          endDateRental: addRemoveMinutes(
      ...
      if (productDetailData?.is_slot_from_dispatch) {
        ...
        try {
          ...
          const res = await actions.checkVendorSlots(
            `?vendor_id=${vendorId}&date=${moment(
              appointmentSelectedDate,
            ).format('YYYY-MM-DD')}&delivery=${dine_In_Type}`,
            {
              code: appData?.profile?.code,
              // currency: currencies?.primary_currency?.id,
              // language: languages?.primary_language?.id,
                ...
                <View
                  ...
                  <View style={{flex: 1, alignItems: 'center'}}>
                    <Banner2
                      autoPlay={false}
                      resizeMode={
                        getBundleId() == appIds?.eatHalal ? 'cover' : 'contain'
                      }
                      bannerRef={bannerRef}
                ...
                <View>
                  ...
                  <Text
                    ...
                    <Text
                      ...
                      {tokenConverterPlusCurrencyNumberFormater(
                        productDetailData?.variant[0]
                          ?.incremental_price_per_min,
                        digit_after_decimal,
                        additional_preferences,
                        currencies?.primary_currency?.symbol,
                      )}{' '}
