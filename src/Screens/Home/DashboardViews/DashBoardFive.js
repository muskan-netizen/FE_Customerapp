      ...
      <View>
        <View style={styles.viewAllVeiw}>
          <Text style={{
            ...
            color: isDarkMode ? MyDarkTheme.colors.text : colors.black,
            marginTop: 0,
          }}>{strings.EXPLORE_STORES}  {appData?.profile?.preferences?.vendors_nomenclature}</Text>

          {!!vendorsData && vendorsData.length > 1 && (
            <TouchableOpacity onPress={() => onViewAll('vendor', appMainData.vendors)}>
              <Text style={styles.viewAllText}>{strings.VIEW_ALL}</Text>
            </TouchableOpacity>
          )}
            ...
            <MenuTrigger>
              <View
                ...
                <Text
                  ...
                  {!currSelectedFilter ? strings.RELEVANCE : currSelectedFilter?.type}
                </Text>
              </View>
            </MenuTrigger>
            <MenuOptions
