...
export default function SearchProductVendorItem2({ navigation, route }) {
  ...
  const globalSearch = (pageCount, searchAgain = false) => {
    ...
    searchAction(query, data, {
      ...
      updateState({
        searchData: searchAgain ? response.data : [...searchData, ...response.data],
  ...
  useEffect(() => {
    const searchInterval = setTimeout(() => {
      let searchObj = {};
      if (searchInput.trim()) {
        updateState({ searchLoader: true, showRightIcon: true, pageCount: 1, showShimmer: true });
      ...
      if (!!searchObj.search_text) {
        ...
        updateState({ searchData: [], showRightIcon: false, isLoading: false, searchLoader: false, showShimmer: false });
      }
    }, 600);
    return () => {
      if (searchInterval) {
