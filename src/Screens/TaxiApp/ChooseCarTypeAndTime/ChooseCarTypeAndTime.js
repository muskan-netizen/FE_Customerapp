...
export default function ChooseCarTypeAndTime({ navigation, route }) {
    ...
    actions
      .getAllCarAndPrices(
        `/${selectedVendorOption?.id}/${paramData?.id}?page=${pageNo}&limit=${limit}`,
        {
          ...
          schedule_date_delivery: scheduleDateTime?.selectedDateAndTime ? scheduleDateTime?.selectedDateAndTime : `${pickedUpDate ? pickedUpDate : ''} ${pickedUpTime ? pickedUpTime : ''}`
        },
        {
  ...
  const _confirmAndPay = () => {
    ...
    data['schedule_time'] =
      scheduleDateTime?.selectedDateAndTime ? `${scheduleDateTime?.selectedDateAndTime}` : pickUpTimeType == 'now' ? '' : slectedDate && selectedTime && `${slectedDate} ${selectedTime}`;
  ...
  const _onDateChange = (date) => {
    ...
    console.log(dateSelectd, time, 'dateSelectd');
    updateState({
      scheduleDateTime: {
  ...
  const _selectPaymentView = () => {
    return (
      <SelectPaymentModalView
        ...
        slectedDate={scheduleDateTime?.slectedDate ? scheduleDateTime?.slectedDate : pickedUpDate}
        isModalVisible={isModalVisible}
        selectedTime={scheduleDateTime?.selectedTime ? scheduleDateTime?.selectedTime : pickedUpTime}
        date={date}
        onPressBack={() =>
          redirectFromNow
            ? updateState({ showCarModal: true, showPaymentModal: false })
        ...
        {!!showCarModal && (
          <View
            ...
            {availableCarList.length > 0 && (<GradientButton
              ...
              btnText={
                `${scheduleDateTime?.selectedDateAndTime ? `${scheduleDateTime?.selectedDateAndTime}` : slectedDate || selectedTime ? `${slectedDate} ${selectedTime}` : 'Schedule a ride'}`
              }
