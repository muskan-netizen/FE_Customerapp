    ...
    BluetoothManager.checkBluetoothEnabled().then(async (enabled) => {
      ...
      if (enabled && isConnected) {
        ...
        try {
          ...
          const base64Data = await getBase64Image(`${detail.admin_profile.logo.image_fit}200/200${detail.admin_profile.logo.image_path}`)

          await BluetoothEscposPrinter.printPic(base64Data.base64String, { width: 200, left: 0 });
          ...
          await detail.vendors[0].products.forEach(async (el) => {
            ...
            BluetoothEscposPrinter.printColumn(columnWidths,
              [BluetoothEscposPrinter.ALIGN.LEFT, BluetoothEscposPrinter.ALIGN.RIGHT],
              [`${el.quantity} X ${title}`, JSON.stringify(el.quantity * el.price)], {});
