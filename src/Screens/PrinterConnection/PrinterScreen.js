...
class PrinterScreen extends Component {
  ...
  componentDidMount = async() => {
    ...
    if (Platform.OS === 'ios') {
      ...
      this._listeners.push(bluetoothManagerEmitter.addListener(BluetoothManager.EVENT_CONNECTION_LOST, async () => {
        this.setState({
          name: '',
          boundAddress: ''
        });
      ...
      this._listeners.push(DeviceEventEmitter.addListener(
        BluetoothManager.EVENT_CONNECTION_LOST, async () => {
          this.setState({
            name: '',
            boundAddress: ''
          });
  ...
  _renderPairedRow(rows) {
    ...
    for (let i in rows) {
      ...
      if (row.address) {
        items.push(
          <TouchableOpacity key={new Date().getTime() + i} style={styles.PairedRowView} onPress={async () => {
            this.setState({
              loading: true
            });
            ...
            AsyncStorage.getItem('BleDevice').then(res => {
              // console.log('checking ble device storage data >>>', JSON.parse(res))
              if (res !== null) {
                BluetoothManager.disconnect(JSON.parse(res).boundAddress).then(s => {
            ...
            <View style={styles.rowContainer}>
              <View style={styles.rowContainerInner}>
                ...
                <View>
                  <Text style={styles.PairedRowName}>{row.name || "UNKNOWN"}</Text>
        ...
        <KeyboardAwareScrollView
          ...
          <View style={styles.main}>
            <ScrollView style={styles.container}>
              ...
              <Text style={styles.title}>Connected:<Text style={{ color: colors.themeColor }}>{!this.state.name ? ' No Devices' : this.state.name}</Text></Text>
              {
                !(this.state.loading || !(this.state.bleOpend && this.state.boundAddress.length > 0))
                && (
                  <TouchableOpacity
              ...
              <View style={{ flex: 1, flexDirection: "column", paddingBottom: 100 }}>
                {
