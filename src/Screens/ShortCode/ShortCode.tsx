...
export default function ShortCode() {
  ...
  const initApiHit = async () => {
    ...
    actions.initApp({}, header, false, null, null, true)
      ...
      .catch(error => {
        ...
        setTimeout(() => {showError(error?.message || error?.error)}, 500);
      });
  };
  ...
  const _renderSplash = useCallback(() => {
    switch (getBundleId()) {
      case appIds.masa: return animatedSplash();
      case appIds.muvpod: return animatedSplash();
      case appIds.hezniTaxi: return animatedSplash();
      case appIds.parcelworks: return animatedSplash();
      default: return imageSplash();
    }
  }, []);
  const animationVideo = useCallback(() => {
    switch (getBundleId()) {
      case appIds?.masa: return imagePath.masa;
      case appIds?.muvpod: return imagePath.muvpod;
      case appIds?.hezniTaxi: return imagePath.HezniSplash; 
      case appIds?.parcelworks: return imagePath.parcelWorksSplash;
    }
  }, [])
  ...
  const onVideoDurationEnded = useCallback(() => {checkNavigationState(true)}, [])
  const checkNavigationState = (videoEnd: boolean) => { if (videoEnd) { navigateToNextScreen(apiRes)}};
